...
module.exports = kconfig = async (kill, message) => {
	...
	const isOwner = isOwn.includes(ownerNumber)
		...
		case 'tempest':
			...
			kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/I8Pdhhuuxg0BiMuwIhbKJK', 'Que otimo que se interessou pelo Tempest Z!\nAi está nosso grupo!', id)
			break
       ...
       case 'translate':
        ...
        case 'criador':
            kill.sendContact(from, '556296638900@c.us')
      ...
      case 'moddroid':
        ...
        case 'exclusive':
			...
			if (args[0] == 'on') {
                ...
                kill.reply(from, 'Os comandos exclusivos do Familia Tempest foram habilitados.', id)
			} else if (args[0] == 'off') {
				let exclu = exsv.indexOf(chatId)
                exsv.splice(exclu, 1)
                fs.writeFileSync('./lib/exclusive.json', JSON.stringify(exsv))
                kill.reply(from, 'Os comandos exclusivos do Familia Tempest foram desabilitados.', id)
            } else {
